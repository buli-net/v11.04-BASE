...
public abstract class SendCoinsOfflineTask {
    ...
    public final void sendCoinsOffline(final SendRequest sendRequest) {
        backgroundHandler.post(() -> {
            ...
            try {
                ...
                if (x.missing!= null) log.info("send failed, {} missing", x.missing.toFriendlyString());
    ...
    private Transaction sweepTaproot(SendRequest sendRequest) throws Exception {
        ...
        // Fee from original request, no hardcoded fee.
        // feePerKb is set by SweepWalletFragment.
        long feePerKb = (sendRequest.feePerKb!= null)? sendRequest.feePerKb.value : Transaction.DEFAULT_TX_FEE.value;
        long fee = feePerKb * 150 / 1000; // 150 vB estimated for 1 P2TR input
    ...
    private byte[] calcSighash(Transaction tx, int idx, List<UTXO> utxos) throws Exception {
        ...
        byte[] ha = sha(sha(bos.toByteArray())); bos.reset();
        ...
        byte[] hseq = sha(sha(bos.toByteArray())); bos.reset();
        ...
        writeU32(ss, tx.getVersion()); writeU32(ss, tx.getLockTime());
        ss.write(hp); ss.write(ha); ss.write(hs); ss.write(hseq); ss.write(hout);
        ss.write(0x00); writeU32(ss, idx);
    ...
    private byte[] signSchnorr(ECKey priv, byte[] m32) throws Exception {
        ...
        eb.write(rX); eb.write(pX); eb.write(m32);
    ...
    private byte[] tagged(String t, byte[] m) throws Exception {
        ...
        sha.reset(); sha.update(th); sha.update(m);
        return sha.digest();
    }
    ...
    private void writeCompact(ByteArrayOutputStream os, long v) {
        if (v < 253) os.write((int) v);
        else if (v < 0x10000) { os.write(253); writeU32(os, v); }
        else { os.write(254); writeU64(os, v); }
    }
