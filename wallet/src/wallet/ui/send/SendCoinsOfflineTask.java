...
public abstract class SendCoinsOfflineTask {
    ...
    public final void sendCoinsOffline(final SendRequest sendRequest) {
        backgroundHandler.post(() -> {
            ...
            try {
                ...
                // FIX TAPROOT: nếu ví giấy là bc1p thì wallet.sendCoinsOffline gốc sẽ throw, ta fallback sang ký Taproot
                ...
                // thêm để không văng app như ảnh trước
    ...
    // --- CHỈ THÊM MỚI TỪ ĐÂY, KHÔNG SỬA GÌ Ở TRÊN ---
    ...
    private Transaction sweepTaproot(SendRequest sendRequest) throws Exception {
        ...
        // fee giữ nguyên cơ chế gốc: dùng feePerKb của request
        long fee = 1000;
        ...
        try { keys.addAll(wallet.getImportedKeys()); } catch (Throwable ignore) {}
    ...
    private void setScriptSig(TransactionInput in, Script s) throws Exception {
        try { Method m = TransactionInput.class.getMethod("setScriptSig", Script.class); m.invoke(in, s); return; } catch (Throwable e) {}
        try { Method m = TransactionInput.class.getDeclaredMethod("setScriptSig", Script.class); m.setAccessible(true); m.invoke(in, s); return; } catch (Throwable e) {}
        Field f = TransactionInput.class.getDeclaredField("scriptBytes"); f.setAccessible(true); f.set(in, s.getProgram());
    }
    private void setWitness(TransactionInput in, TransactionWitness w) throws Exception {
        try { Method m = TransactionInput.class.getMethod("setWitness", TransactionWitness.class); m.invoke(in, w); return; } catch (Throwable e) {}
        try { Method m = TransactionInput.class.getDeclaredMethod("setWitness", TransactionWitness.class); m.setAccessible(true); m.invoke(in, w); return; } catch (Throwable e) {}
        try { Field f = TransactionInput.class.getDeclaredField("witness"); f.setAccessible(true); f.set(in, w); return; } catch (Throwable e) {}
        Method m2 = Transaction.class.getMethod("setWitness", int.class, TransactionWitness.class); m2.invoke(in.getParentTransaction(), in.getIndex(), w);
    }
    private ECKey deriveTweaked(ECKey k) throws Exception {
        byte[] comp = k.getPubKey(); boolean odd = (comp[0] & 1) == 1;
        ...
        BigInteger n = spec.getN(); BigInteger d = k.getPrivKey(); if (odd) d = n.subtract(d);
        byte[] xOnly = new byte[32]; System.arraycopy(comp, 1, xOnly, 0, 32);
        MessageDigest sha = MessageDigest.getInstance("SHA-256"); byte[] tag = sha.digest("TapTweak".getBytes(java.nio.charset.StandardCharsets.UTF_8));
        sha.reset(); sha.update(tag); sha.update(xOnly); BigInteger tweak = new BigInteger(1, sha.digest());
    ...
    private byte[] calcSighash(Transaction tx, int idx, List<UTXO> utxos) throws Exception {
        ...
        byte[] hp = sha(sha(bos.toByteArray())); bos.reset();
        for (UTXO u : utxos) writeU64(bos, u.getValue().value); byte[] ha = sha(sha(bos.toByteArray())); bos.reset();
        for (UTXO u : utxos) { byte[] spk = u.getScript().getProgram(); writeCompact(bos, spk.length); bos.write(spk); }
        byte[] hs = sha(sha(bos.toByteArray())); bos.reset();
        for (int i = 0; i < utxos.size(); i++) writeU32(bos, TransactionInput.NO_SEQUENCE - 2); byte[] hseq = sha(sha(bos.toByteArray())); bos.reset();
        ...
        ByteArrayOutputStream ss = new ByteArrayOutputStream(); ss.write(0x00); ss.write(0x00); writeU32(ss, tx.getVersion()); writeU32(ss, tx.getLockTime());
        ss.write(hp); ss.write(ha); ss.write(hs); ss.write(hseq); ss.write(hout); ss.write(0x00); writeU32(ss, idx);
        return tagged("TapSighash", ss.toByteArray());
    }
    private byte[] signSchnorr(ECKey priv, byte[] m32) throws Exception {
        org.bouncycastle.jce.spec.ECNamedCurveParameterSpec spec = org.bouncycastle.jce.ECNamedCurveTable.getParameterSpec("secp256k1");
        BigInteger n = spec.getN(); BigInteger d = priv.getPrivKey(); if ((spec.getG().multiply(d).normalize().getEncoded(true)[0] & 1) == 1) d = n.subtract(d);
        BigInteger k = null; org.bouncycastle.math.ec.ECPoint R = null;
        do { byte[] rnd = new byte[32]; secureRandom.nextBytes(rnd); k = new BigInteger(1, rnd).mod(n); if (k.signum() == 0) continue; R = spec.getG().multiply(k).normalize(); if ((R.getEncoded(true)[0] & 1) == 1) k = n.subtract(k); } while (k == null || k.signum() == 0 || R == null);
        byte[] rX = new byte[32]; System.arraycopy(R.getEncoded(true), 1, rX, 0, 32); byte[] pX = new byte[32]; System.arraycopy(spec.getG().multiply(d).normalize().getEncoded(true), 1, pX, 0, 32);
        ByteArrayOutputStream eb = new ByteArrayOutputStream(); eb.write(rX); eb.write(pX); eb.write(m32); BigInteger e = new BigInteger(1, tagged("BIP0340/challenge", eb.toByteArray())).mod(n);
        BigInteger s = k.add(e.multiply(d)).mod(n); ByteArrayOutputStream sig = new ByteArrayOutputStream(); sig.write(rX); byte[] sb = s.toByteArray(); byte[] s32 = new byte[32];
        if (sb.length > 32) System.arraycopy(sb, sb.length - 32, s32, 0, 32); else System.arraycopy(sb, 0, s32, 32 - sb.length, sb.length); sig.write(s32); return sig.toByteArray();
    }
    private byte[] tagged(String t, byte[] m) throws Exception { MessageDigest sha = MessageDigest.getInstance("SHA-256"); byte[] th = sha.digest(t.getBytes(java.nio.charset.StandardCharsets.UTF_8)); sha.reset(); sha.update(th); sha.update(m); return sha.digest(); }
    private byte[] sha(byte[] b) throws Exception { return MessageDigest.getInstance("SHA-256").digest(b); }
